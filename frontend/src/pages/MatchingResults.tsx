...
import { Users, Star, IndianRupee, MapPin, CheckCircle, Clock, AlertTriangle, ShieldCheck, UserPlus, RefreshCw } from 'lucide-react';
...
export const MatchingResults = () => {
  ...
  useEffect(() => {
    const fetchOptimization = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE}/crew/optimize/${eventId}`, {
  ...
  const handleConfirmCrew = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/crew/confirm`, {
        method: 'POST',
        headers: {
  ...
  const handleSimulateNoShow = async (workerId: string) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/crew/replace-worker`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${user.token}`
      ...
      if (response.ok) {
        ...
        fetch(`${API_BASE}/crew/optimize/${eventId}`, {
            headers: { 'Authorization': `Bearer ${user.token}` }
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 relative">
      ...
      <div className="mb-8 border-b border-slate-200 pb-6">
        ...
        {(() => {
          ...
          results.forEach(r => {
             const selected = selectedWorkers[r.role.id] || [];
             primaryCount += selected.length;
             r.candidates.forEach((c: any) => {
                totalCandidates++;
                if (selected.includes(String(c.id))) {
                   totalReliability += c.reliabilityScore;
                   estimatedCost += (c.priceMin + c.priceMax) / 2;
                } else {
                   backupCount++;
                }
             });
          ...
          const avgReliability = primaryCount > 0 ? Math.round(totalReliability / primaryCount) : 0;

          return (
             <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl shadow-sm">
                   <p className="text-xs font-bold text-indigo-500 uppercase tracking-wider mb-1">Evaluated</p>
                   <p className="text-2xl font-black text-indigo-900">{totalCandidates}</p>
                </div>
                <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl shadow-sm">
                   <p className="text-xs font-bold text-emerald-500 uppercase tracking-wider mb-1">Primary Crew</p>
                   <p className="text-2xl font-black text-emerald-900">{primaryCount}</p>
                </div>
                <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl shadow-sm">
                   <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Backups</p>
                   <p className="text-2xl font-black text-slate-900">{backupCount}</p>
                </div>
                <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl shadow-sm">
                   <p className="text-xs font-bold text-blue-500 uppercase tracking-wider mb-1">Avg Reliability</p>
                   <p className="text-2xl font-black text-blue-900">{avgReliability}%</p>
                </div>
                <div className="bg-purple-50 border border-purple-100 p-4 rounded-xl shadow-sm">
                   <p className="text-xs font-bold text-purple-500 uppercase tracking-wider mb-1">Budget Used</p>
                   <p className="text-2xl font-black text-purple-900">₹{estimatedCost.toLocaleString()}</p>
                </div>
             </div>
          );
        })()}
                ...
                <div className="space-y-4 mb-8">
                  {primaryCrew.length === 0 ? (
                    ...
                    primaryCrew.map((worker: any) => (
                      <div 
                        key={worker.id} 
                        className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 rounded-xl border-2 border-indigo-500 bg-gradient-to-r from-indigo-50/50 to-white shadow-sm transition-all relative overflow-hidden"
                      >
                        <div className="absolute top-0 left-0 w-1.5 h-full bg-indigo-500"></div>
                        <div className="flex-1 mb-4 md:mb-0 pl-2">
                          <div className="flex items-center space-x-3 mb-3">
                            <h3 className="text-xl font-extrabold text-slate-900">{worker.name}</h3>
                            <span className="px-2 py-1 text-xs font-black bg-emerald-100 text-emerald-800 rounded-full border border-emerald-200 flex items-center">
                              <CheckCircle className="w-3 h-3 mr-1" /> PRIMARY
                            </span>
                            <div className="flex items-center space-x-1.5 bg-indigo-100 px-3 py-1 rounded-full border border-indigo-200">
                                <span className="text-[10px] font-bold text-indigo-700 uppercase tracking-wider">AI Match</span>
                                <span className="text-sm font-black text-indigo-900">{worker.score}%</span>
                            </div>
                          </div>
                ...
                <div className="space-y-4">
                  {backupCrew.length === 0 ? (
                    ...
                    backupCrew.map((worker: any, index: number) => (
                      <div 
                        key={worker.id} 
                        className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 rounded-xl border border-slate-200 bg-slate-50/50 hover:bg-white hover:border-slate-300 hover:shadow-sm transition-all"
                      >
                        <div className="flex-1 mb-4 md:mb-0">
                          <div className="flex items-center space-x-3 mb-3">
                            <h3 className="text-lg font-bold text-slate-700">{worker.name}</h3>
                            <span className="px-2 py-1 text-xs font-bold bg-slate-200 text-slate-600 rounded-full border border-slate-300">Backup #{index + 1}</span>
                            <div className="flex items-center space-x-1.5 bg-white px-3 py-1 rounded-full border border-slate-200 shadow-sm">
                                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">AI Match</span>
                                <span className="text-sm font-black text-slate-700">{worker.score}%</span>
                            </div>
                          </div>
